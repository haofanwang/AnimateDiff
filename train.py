...
from animatediff.data.dataset import MultiTuneAVideoDataset
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if train_whole_module:
        print("train_whole_module:", True)
    ...
    pixel_transforms = transforms.Compose([transforms.Normalize(mean=[0, 0, 0, 0], std=[1, 1, 1, 1], inplace=True)])
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.interpolation:
                    init_latents = copy.deepcopy(latents[:,:,0,:,:]) # [1,4,64,64]
                    last_latents = copy.deepcopy(latents[:,:,-1,:,:]) # [1,4,64,64]

                    # make the frozen latents have the same scale as noise ~N(0,1)
                ...
                if args.interpolation:
                    # recover the first and last latents
                    noisy_latents[:,:,0,:,:] = init_latents
                    noisy_latents[:,:,-1,:,:] = last_latents
                ...
                if args.interpolation:
                    loss = F.mse_loss(model_pred.float()[:,:,1:-1,:,:], target.float()[:,:,1:-1,:,:], reduction="mean")
                else:
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        unwarp_unet = accelerator.unwrap_model(unet)
                        save_path = os.path.join(output_dir, f"mm-{global_step}.pth")
                        save_checkpoint(unwarp_unet, save_path)
